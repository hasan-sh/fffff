...
def url_to_string(url):
    ...
    return html_to_string(parser_content), parser_content
...
def parse_keyword(html_content, keyword='author', byClass='byline', notmeta = False):
    ...
    if found_author and not notmeta:
        name = found_author['content']
   
   # If the author name cannot be found in the metadata, we might find it as an attribute of the text.
    else:
        itemprop = html_content.find(attrs={'itemprop': keyword})
        byline = html_content.find(attrs={'class': byClass})
    
        found_author = itemprop or byline
        
        if found_author:
            if 'time' in keyword:
                name = found_author.time["datetime"]
                if not name:
                    print(found_author, "HERE")
            else:
...
def parse_news_text(html_content):
    ...
    if not article:
        ...
        if not articles:
            articles = html_content.find_all('div.content-lock-content')
        if articles:
